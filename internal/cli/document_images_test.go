...
func TestDocCommandsForImages(t *testing.T) {
	...
	added, err := runCLI(t, "--db", path, "doc", "add", "DOC-1", "--file", photo)
	if err != nil {
	...
	shown, err := runCLI(t, "--db", path, "doc", "show", "img_0042.jpeg", "--ticket", "DOC-1")
	if err != nil {
	...
	downloaded, err := runCLI(t, "--db", path, "doc", "download", id, "--output", out)
	if err != nil {
	...
	// To standard output.
	piped, err := runCLI(t, "--db", path, "doc", "download", id, "-o", "-")
	if err != nil {
	...
	saved, err := runCLI(t, "--db", path, "doc", "write", id, "--file", replacement)
	if err != nil {
	...
	shown, _ = runCLI(t, "--db", path, "doc", "show", id)
	...
	for _, tc := range []struct {
		...
		printed, err := runCLIWithInput(t, string(tc.input), append([]string{"--db", path, "doc", "add"}, tc.args...)...)
		if err != nil {
