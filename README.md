...
- **Views** — the sidebar's Views group shows the same tickets three ways:
  - **Dependencies**, the home page at `/`, puts open tickets in columns by how many steps of unfinished work block them, with arrows from each blocker and the edge that closes a cycle drawn in red. Hover a card to trace what blocks it and what it blocks; pan, zoom or fit the graph to the screen
